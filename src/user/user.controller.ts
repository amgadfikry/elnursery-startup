...
export class UserController {
  ...
  @ApiOperation({ summary: 'Create a new user account and send email with password (admin only)' })
  ...
  @UseInterceptors(new TransformInterceptor(ReturnedUserDto)) // Transform the response to the ReturnedUserDto
  @ApiOperation({ summary: 'Get all user accounts either all or filtered by query parameters classCategory (admin only)' })
  ...
  @UseInterceptors(new TransformInterceptor(ReturnedUserDto)) // Transform the response to the ReturnedUserDto
  @ApiOperation({ summary: 'Get the user account by id or get own details without id if current user is user (admin/user)' })
  ...
  @UserType('admin') // Check if the user type is admin
  @UseInterceptors(new TransformInterceptor(ReturnedUserDto)) // Transform the response to the ReturnedUserDto
  @ApiOperation({ summary: 'Update the user account by id details (admin only)' })
  ...
  @Delete(':id')
  @UserType('admin') // Check if the user type is admin
  @ApiOperation({ summary: 'Delete the user account by id (admin only)' })
